...
- **Modular rules & docs.** Path-scoped guidance lives in **nested `CLAUDE.md`
  files** — `src/CLAUDE.md`, `src/components/CLAUDE.md`,
  `src/components/organisms/Mapbox/CLAUDE.md`, `src/config/CLAUDE.md`,
  `src/config/api/CLAUDE.md` — each of which is included when Claude reads files
  in that directory. Reference docs live in `docs/*.md`. Skim the nested guide
  for a subsystem before editing it.
...
Two test lanes (see `CLAUDE.md § Testing`): a **fast node-only unit lane**
...
## Conventions (see the nested `CLAUDE.md` files under `src/` for detail)
...
- **State**: zustand stores (`src/config/store.ts`) are the single source of truth
  ...
  `useShallow` selectors in hot paths (the map). See `src/config/CLAUDE.md`.
- **Navigation**: the UI is a **URL-driven drawer stack** (`src/views/`).
  ...
  popping the host page's history. See `src/config/CLAUDE.md`.
- **Map containment**: map mode fills the viewport only when the host gave `<sahaj-atlas>` no
  ...
  mechanism, pointed at a box the host owns. See `src/components/CLAUDE.md`.
- **Layout**: when the interface does not fit, `AppShell` renders a **compact card** — one
  ...
  There is deliberately **no override parameter**. See `src/components/CLAUDE.md`.
- **Responsive**: the widget adapts to **its own box**, not the browser window
  ...
  container-query plugin are in `src/components/CLAUDE.md`;
...
- **Map**: layer definitions live in `src/components/organisms/Mapbox/layers.ts`;
  ...
  `map=false` — so no view branches on whether a map exists. See `src/components/organisms/Mapbox/CLAUDE.md`.
- **Components**: atomic tiers (`atoms/molecules/organisms`), **PascalCase
  ...
  `src/components/CLAUDE.md`. Preview components with `pnpm ladle`.
