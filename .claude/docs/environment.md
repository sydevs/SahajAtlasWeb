...
| `ATLAS_API_KEY`            | your shell   | A **production** `sahaj-atlas-client` API key, used **only** by `pnpm sync:atlas-languages` to read the operator-owned language set off SahajCloud's `sy-atlas-config` global (#167). It falls back to `VITE_SAHAJCLOUD_API_KEY` **only when `VITE_SAHAJCLOUD_URL` is production too** — otherwise that key belongs to a local backend, and sending it would put a credential for one environment into another's request logs to earn a 403. The script deliberately ignores `VITE_SAHAJCLOUD_URL` for the same reason: a snapshot of seeded dev data, asserted against in CI, is worse than no snapshot                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
