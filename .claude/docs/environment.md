...
| `VITE_SENTRY_DSN`           | Cloudflare   | Sentry ingest DSN for automatic error reporting (issue #108). Public by design — a DSN is write-only and ships in every client bundle — but deliberately NOT committed to `.env`: a DSN in the repo means every fork, preview and developer's `pnpm dev` posts into the production project. Set it per-environment in the Cloudflare Pages dashboard. **Unset ⇒ `reportInternalError` logs to the console and nothing else; the SDK chunk is never fetched.** Hosts can also decline it per-embed with `error-reporting="false"` |
