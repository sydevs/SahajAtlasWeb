...
| `ExpandedSurface/`    | `ExpandedSurface`                                    | Radix Dialog with no chrome but its collapse control: `fixed inset-0`, the whole viewport — what a compact embed expands into (issue #161). Map mode's layout made explicit, which is why vaul's window-height arithmetic is correct inside it. Adopts itself as `overlayContainer()` while open, so every portal in the app lands inside the focus trap. Not the `Modal` atom: that is a `max-w-md` panel with a header |
...
| `CompactCard/`         | `DynamicCompactCard`                                       | The whole widget in a slot too small for it: one TASK-named button into the `ExpandedSurface`, with up to three `EventListItem` rows above it when `compactFit` says the host's box has room. Content-height when the element has no height of its own; centred both ways when it does. The presentational `CompactCard` is module-private (single-use). Non-suspense — an empty list is a state, not an error |
