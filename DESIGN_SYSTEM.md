...
| `SearchableSelect/`   | `SearchableSelect`                                   | Type-to-filter single-select combobox (on `usePopover` + a filter input). The Region filter — `Select` has no typeahead, `Dropdown` is a checkbox list |
