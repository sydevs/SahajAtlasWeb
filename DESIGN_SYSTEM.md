...
| `CompactCard/`         | `DynamicCompactCard`                                       | The whole widget in a slot too small for it: a heading, up to three `EventListItem` rows off the cached feed, and one TASK-named button into the `ExpandedSurface`. The presentational `CompactCard` is module-private (single-use). Non-suspense — an empty list is a state, not an error |
