...
| `SortMenu/`          | `SortMenu`                         | The results-list sort selector (Recommended / Closest / Soonest), persisted in `?sort=`; a Radix DropdownMenu radio group (mirrors `SettingsMenu`). `iconOnly` swaps the labelled trigger for the drawer-header icon control — how SearchView renders it, beside the Filters icon |
