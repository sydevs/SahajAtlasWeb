...
//
// The other two specs fetch the standalone page (`index.html`), which is the dev and demo
// surface. The product is the embed — and since #149 the file a host installs is `auto.js`,
// the loader, which fetches `embed.js` on demand. So the crawl below starts at the loader,
// rather than the widget: starting at `embed.js` would still prove the widget deployed, while
// missing the one file every host actually references. A build or deploy that emitted a broken
// embed while keeping the standalone page healthy would otherwise ship past a green Smoke
// check (issue #99).
//
// This lane stays fetch-level only, deliberately: it is fetch-based by design
// (`docs/testing.md`), and booting the widget in a real browser belongs to local Playwright
// verification, not CI.

/**
 * The embed bundle, fetched once and shared. Both specs need the same body, and `retry: 2`
 * in the smoke config would otherwise triple the round trips. This stays lazy, rather than a
 * `beforeAll`, so a run with no preview URL skips without ever touching the network.
 *
...
describe('embed bundle', () => {
  test.skipIf(skipWithoutPreview)('serves auto.js as a loader, not as the widget', async () => {
    ...
    // This is the seam the whole split rests on, asserted on the deployed artifact rather
    // than only in the build: the widget must be reached through a DYNAMIC import. If
    // someone makes it static, the loader stops being a loader — every host silently goes
    // back to paying the full payload on every page view, and nothing else here would
    // notice.
  ...
  test.skipIf(skipWithoutPreview)('serves every chunk the loader can reach', async () => {
    ...
    //
    // This matches every relative `.js` literal, not just the static-import shape: a lazy
    ...
    // The views behind a lazy seam are reached from a chunk it names, rather than the
    // entry — so a one-level crawl stopped covering the calendar, registration, and share
    // chunks the moment those moved behind `React.lazy` (issue #96). Following each body
    // means the set is the whole deploy the widget can ever ask for, however deep the
    // seam.
    // BACKTICKS matter as much as the transitivity. rolldown's minifier emits dynamic
    // specifiers as template literals — `import(\`./CalendarView-<hash>.js\`)` — so a
    // class of only `"` and `'` matched the static imports and silently skipped every
    // lazy chunk in the build. The comment above has claimed otherwise since this spec
    // was written.
    ...
    while (queue.length) {
      ...
      for (const [, spec] of source.matchAll(RELATIVE_JS)) {
        ...
        // deploy is roughly 440 KiB gzipped, and this lane runs separately from the PR
        // gate.
