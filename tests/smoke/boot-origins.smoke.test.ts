...
// `embed.smoke.test.ts`). It does not need one here: the defect is a STRING IN THE BUNDLE, so
// reading the bundle back is a direct observation of it, rather than an inference. That also
// makes the test deterministic — it cannot pass just because a runner happened to have
// something listening on 5174.

/** The `assets/*.js` the standalone entry pulls in up front — its `<script>` plus modulepreloads. */
 ...
 *
 * ⚠ **This is targeted, rather than a sweep for any private host, and the difference is a
 * false positive that a first draft of this spec hit.** react-router carries a literal
 ...
 * fourteen round trips. This stays lazy, so a run with no preview URL never touches the
 * network, and a failure is NOT cached — the preview is an edge deploy published seconds
 * earlier, which is exactly when a transient 5xx happens, and re-awaiting a settled
 * rejection would make all three attempts identical and instant. Same reasoning as
 * `embed.smoke.test.ts`.
 */
...
/**
 * Reads each request origin back out of the shipped bundle.
 *
 * This fails, rather than returns nothing, when one cannot be found: an origin that stops
 * matching — a minifier change, a refactor that composes the URL differently — would
 * otherwise make every assertion below vacuously true, which is the failure mode this
 * lane's own rules warn is hardest to notice, because nothing about it looks like one.
 */
...
describe('boot origins', () => {
  ...
  test.skipIf(skipWithoutPreview)(
    ...
    async () => {
      // Reads the origin out of the bundle, rather than assuming it is the preview's own.
      // Both answers are legitimate — production bakes in `sahajatlas.com`, a preview its
      // own deployment host — so what has to hold is that whatever was baked in RESOLVES.
      //
      // ⚠ **This is NOT the guard for a localhost origin, and must never be treated as
      // one.** Dialling the baked origin is environment-dependent: anyone running
      // `pnpm dev` has something answering on 5174, so against the very deploy that
      // shipped this bug it passed on a developer's machine and would only have failed on
      // a runner. This is verified, not supposed. That case belongs to the private-host
      // test above, which READS the string rather than dialling it and so cannot be
      // fooled by what is listening locally. What this test owns is the other half: an
      // origin that is public and simply wrong — a typo, a retired host — which no amount
      // of string inspection would catch.
      ...
      // This is parsed, not just checked for 200: `_redirects` is `/* /index.html 200`,
      // so a missing file answers 200 `text/html`, and only the body can tell the
      // difference. This is the lane's second invariant — status is not a result.
  ...
  test.skipIf(skipWithoutPreview)(
    ...
    async () => {
      // The deploy must also SERVE them, independently of what the bundle asks for — a
      // bundle pointing somewhere valid and a deploy missing `public/locales/` are
      // different faults with the same blank-page symptom.
      for (const ns of ['common', 'events']) {
        ...
        // `text/html` for a file that is not there, so only the body distinguishes the
        // two.
  ...
  test.skipIf(skipWithoutPreview)('serves those translations CORS-open', async () => {
    // Embedded, the widget runs on the host's page and fetches locales cross-origin, so
    // the `/locales/*` rule in `public/_headers` is what stands between a working embed
    // and one rendering every string as its raw dotted key (issue #91).
    // `robots.smoke.test.ts` pins that rule against displacement. This pins that it is
    // doing its job on the real file.
