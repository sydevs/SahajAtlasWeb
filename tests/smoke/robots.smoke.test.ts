...
// `public/robots.txt` and `public/_headers` are inert files in the repo until Cloudflare
// Pages reads them out of the build output, and `_headers` in particular is a platform
// behaviour we assert, rather than code we wrote.

/**
 * The static tag in index.html. This ignores attribute order on purpose: the assertion is
...
describe('indexing directives', () => {
  test.skipIf(skipWithoutPreview)('serves robots.txt with a site-wide Disallow', async () => {
    ...
    // `/* /index.html 200`, so a robots.txt missing from the build comes back as the
    // SPA shell with a 200, which is the failure this spec exists to catch.
  ...
  // fallback resolves. Both return the same shell, so this test does not check the
  // header rule's path matching — it checks that a rewrite does not drop either
  // signal on the routes people are actually linked to.
  ...
  test.skipIf(skipWithoutPreview)('serves noindex on the non-document URLs too', async () => {
    ...
    // `/*` has stopped covering anything the meta does not already cover.
  ...
  test.skipIf(skipWithoutPreview)('adds the header without displacing the CORS rules', async () => {
    // This is the regression this PR could realistically cause. `_headers` already
    // carried `/assets/*` and `/locales/*` CORS rules (issue #91 — a font is always
    // fetched in CORS mode, and blocked locale JSON renders every string as its raw
    // key). Adding `/*` is only safe because Pages applies EVERY matching rule. This
    // asserts that, rather than trusting the doc quote in the comment there.
