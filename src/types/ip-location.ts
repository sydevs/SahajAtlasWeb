...
// (`min(1)`) — so the suggestion simply does not render. `country` is the display
...
// coordinates and cap the strings (a hostile or compromised response cannot push a
...
export const IpLocationSchema = z.object({
  ...
  // — so the label never asserts a place whose local time is not the one displayed.
