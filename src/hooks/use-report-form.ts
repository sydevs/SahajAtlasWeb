...
/**
 * The authored form, whether this atlas offers the report path at all, and whether the form is
 * still on its way.
 *
 ...
 * first.
 *
 * `enabled` gates every entry point — the settings row directly, and the error fallbacks through
 * `visibleActions`' `canReport` limit. It answers off the CONFIG alone, a round trip ahead of the
 * form, which is the gap `isPending` exists to describe. A caller reading only `enabled` costs no
 * extra fetch: the modal host above has the form query mounted either way.
 *
 * `isPending` is what keeps "not here yet" apart from "could not be read" — see
 * `reportFormPending`.
 */
export const useReportForm = (): {
  form: ReportForm | undefined
  enabled: boolean
  isPending: boolean
} => {
  ...
  return {
    form: data,
    enabled: formId !== null,
    isPending: reportFormPending({ configPending, formId, formPending }),
  }
}
