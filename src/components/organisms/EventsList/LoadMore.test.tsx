...
// This mocks the i18n boundary (react-i18next), so the SSR markup asserts on
// real copy — including the Ruby-style `%{}` interpolation — without booting
// i18next. This runs in the node lane, with no jsdom (see docs/testing.md).
...
describe('LoadMore', () => {
  ...
  it('says plainly what the boundary crossing does', () => {
    // This is the list's only distance affordance. It has to read as "nearby
    // events have run out," not as the list simply ending.
  ...
  it('marks itself busy while a reveal renders, but never disabled', () => {
    // `disabled` would unfocus the button for the commit where the flag is up,
    // dropping a keyboard user to <body>. Re-enabling does not bring focus
    // back, so every press would silently cost them their place. The parent's
    // `pending` guard is what makes a second press a no-op. Nothing needs
    // disabling to stay correct.
    const html = render({ loading: true })

    expect(html).toContain('aria-busy="true"')
    // This checks the rendered ATTRIBUTE (`disabled=""`), not the substring.
    // The class list carries Tailwind's `disabled:` variants either way.
    expect(html).not.toMatch(/\sdisabled=/)
  })

  it('renders the same static markup whether or not auto-reveal is armed', () => {
    // `auto` only wires an IntersectionObserver in an effect. So the button it
    // observes must exist identically without one. The keyboard and
    // screen-reader path can never depend on a scroll event firing.
  ...
  it('drops the button once everything is revealed', () => {
    ...
    // The live region survives the button it sat beside. The LAST press is
    // the one that unmounts the control, and its announcement has to outlive it.
