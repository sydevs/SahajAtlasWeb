...
// Node-only SSR assertions (see `docs/testing.md`). This pins the WCAG
// 2.2.2 contract from issue #104. A carousel that moves on its own must
// ship a way to stop it. A static one must NOT grow a control for motion
// it never had. Whether pressing it actually halts Swiper is a
// live-instance question the SSR markup cannot answer. The browser
// verifies that instead.
//
// `usePrefersReducedMotion` is left unmocked on purpose. It answers
// "motion is allowed" here without touching `window` (see its docblock).
// So these specs run in the node lane, and the lane would notice if that
...
describe('ImageCarousel', () => {
  ...
  it('opts the control out of vaul dragging, or touch taps would be swallowed', () => {
    ...
    // micro-movement as a drag, and eats the click. Losing this attribute
    // breaks the control on touch only. That is invisible to every other
    // gate. So this spec pins it here, instead of trusting it to review.
