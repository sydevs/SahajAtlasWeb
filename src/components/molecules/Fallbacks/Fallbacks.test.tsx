...
/** A row with its sentence stripped off. This is everything two rows are allowed to share. */
...
// state offers ONLY the actions that could help. `classifyError`
// (src/lib/report.test.ts) decides which kind a *failure* is. This spec
// decides what any kind, failure or empty list, is then allowed to render.
describe('ERROR_POLICY', () => {
  it('offers a retry but never a report for offline', () => {
    // Connectivity is not ours to fix, and the report POST (#80) needs the
    // very network that just failed.
    expect(ERROR_POLICY.offline).toMatchObject({ retry: true, report: false })
  })

  it('offers none of the failure buttons for a dead link', () => {
    // Retrying a region that does not exist fails identically, every time.
    // A dead link's actual recovery is somewhere to go, not a button that
    // repeats the failure.
  ...
  it('gives a dead link and an empty list the same way out', () => {
    // This is the whole argument for one table. A URL that never existed,
    // and a region whose programs have all ended, leave a viewer in
    // exactly the same position. So the rows may differ ONLY in their
    // sentence. If these ever diverge, one of the two has quietly become
    // the worse dead end.
  ...
  it('offers a person, not a button, when a class cannot be joined', () => {
    // This is the one row whose best next step is somebody who can still
    // let you in. Retrying and reporting are both wrong for it. The class
    // is running. It just has no room.
  ...
  it('marks a malfunction red and a dead end neutral', () => {
    // A red banner over "no events found" would tell a viewer the widget is
    // broken, when nothing is. Register drift is invisible in review, so
    // this spec pins it here.
  ...
  it('never offers a network-dependent way out of a network failure', () => {
    // Both the onward link and the geocoder need the connection that just
    // went away. So offering either would only reproduce the failure one
    // press later.
  ...
  it('carries English for every state, for a failure that beat the locale JSON', () => {
    // Nothing is bundled. i18next fetches every namespace over HTTP. So
    // without a default, the very screens this exists for would render
    // the raw key.
    for (const policy of Object.values(ERROR_POLICY)) {
  ...
  it('keeps every fallbackText word-for-word identical to the shipped en copy', () => {
    // These are two hand-maintained copies of the same sentence. They
    // already drifted once. The locale files were reworded, from "that
    // page" to "what you were looking for", while the defaults kept the
    // old wording. Nobody would have noticed. The default only renders
    // when the locale fetch loses a race. So a viewer on a broken
...
describe('visibleActions', () => {
  it('never leaves a viewer with no way out, with or without a reset', () => {
    // This is the case this guards. `not-found` grants none of the
    // failure buttons, because its real recovery is somewhere to go. On
    // the app-level surface, the drawer stack never mounted. So
    // navigating leads nowhere, and the screen would have zero controls.
  ...
  it('leaves a state that promised nothing with nothing', () => {
    // `no-nearby` is a note about the list directly below it, whose own
    // "Show distant events" control IS the way out. Bolting a report CTA
    // onto it would invite reports of a feature working exactly as
    // designed. The never-strand rule would do exactly that, if it fired
    // on emptiness instead of on a broken promise.
  ...
  it('drops the geocoder where the surface already leads with one', () => {
    // SearchView's header IS a geocoder. A second one under the sentence
    // would be the odd thing on the screen. The onward rung survives, so
    // this cannot strand anyone.
    expect(
  ...
  it('leads with the organiser and drops the onward rung when there is one to call', () => {
    // Not both. "See events nearby" beside a number that can still get you
    // into THIS class would offer a consolation prize as an equal.
    expect(
  ...
  it('has a policy for whatever any fixture classifies as', () => {
    // This backs up the Record's exhaustiveness. A kind reachable at
    // runtime with no policy entry would render a fallback with no
    // buttons and an undefined key.
    for (const kind of mockErrorKinds) {
