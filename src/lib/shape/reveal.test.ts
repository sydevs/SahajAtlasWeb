 ...
 * count. This is one matrix, not one each, because both properties are universal. A
...
describe('revealKey', () => {
  ...
  it('changes with anything the events query key changes', () => {
    // Centre, filters, and locale all reach it through the query key. That is the
    // point of deriving from that key, instead of re-listing its parts here.
  ...
  it('changes with the sort, which the query key deliberately omits', () => {
    // Sort reorders the fetched list instead of refetching, so it is absent from the
    // query key. But it changes which events the revealed rows are, so the reveal has
    // to reset on it. That is the one part `revealKey` adds.
...
describe('revealRows', () => {
  ...
  it('offers the far segment once nearby matches run out, continuing the count', () => {
    ...
    expect(result.more).toBe('farther')
    // This continues the count, instead of resetting it. The 20 rows already read stay on screen.
  ...
  it('keeps saying "distant" for every page after the segment is revealed', () => {
    // This is not `'more'`. Everything below the boundary is a distant event, so a
    // bare "Show more" would stop describing what the press actually fetches.
  ...
  it('offers only the crossing for an all-far result set', () => {
    // This is the empty state the "no events within N km" alert explains. The default
    // count already exceeds the (zero-length) nearby segment. That is exactly why the
    // far reveal is its own flag, instead of being derived from the count.
  ...
  it('holds an event across a border to half the distance', () => {
    // Same distance, different countries: the domestic one is nearby, and the foreign
    // one is not. Someone searching Lille does not want Belgian classes ahead of
    // French ones.
  ...
  it('never demotes an event on a country it does not know', () => {
    // No `?cc` (an ocean, a country-less feature), or no address country (every
    // online event): both are judged on distance alone, instead of on a fact we do
    // not have.
  ...
  it('makes no distance cut without a searched place', () => {
    // This ranks from the map centre. There is no place to be "far" from, so the far
  ...
  it('stops at the ceiling rather than offering a press the clamp would undo', () => {
    // A result set past MAX_REVEAL, all of it nearby: with no distant segment there
    // is nothing to reserve for, so the list keeps the full ceiling. The rows stop
    // there, and so does the control. `next` would be clamped straight back on read,
    // so the button would sit there doing nothing.
  ...
  it('keeps the distant segment reachable when the nearby one fills its own ceiling', () => {
    // This is the case issue #98 pinned as a known sharp edge, and #129 fixed. It is
    // the whole reason the budget is per-segment. See the reason in the
    // `MAX_NEARBY_REVEAL` docblock.
    const events = [...near(MAX_REVEAL + 10), ...far(50)]
    const result = reveal(events, { shown: MAX_REVEAL })

    // Nearby paging stops at the reserve, not at the ceiling, with budget still
    // unspent. So the crossing below has somewhere to put its rows.
    ...
    // instead of a re-slice of the same segment. This call feeds the `next` just
    // asserted, so it is literally the press the control offers, not a hand-copy of
    // it.
    const after = reveal(events, result.next!)

    // This checks by the fixture's tag, not by re-deriving the distance rule the module owns.
  ...
  it('spends the reserve on distant rows, then stops at the ceiling', () => {
    // This pages on past the crossing. The distant segment draws on what the nearby
    // ceiling left, and the list ends at `MAX_REVEAL`, the bound the DOM budget is
    // about.
  ...
  it('never renders more rows than the ceiling, whatever the segment mix', () => {
    // This treats the DOM bound as a property of the function, not just of the
    // constant. The reserve splits the budget between segments, and must never let
    // their sum exceed it.
  ...
  it('still offers the distant segment when a huge count only filled the nearby one', () => {
    // The ceiling is about rows rendered, not the number in the URL. A count past
    // both the ceiling and the nearby segment renders that segment and no more. So
    // the distant events are still one press away, not silently unreachable.
  ...
  it('never offers a press that would reveal nothing', () => {
    ...
    // The assertion above sits inside a conditional inside a loop. Without this check,
    // a sweep that never reached a state with a press on offer would pass having
    // checked nothing.
    expect(offered).toBeGreaterThan(0)
  })
})

// This is a ratchet on the constants, not on `revealRows`, hence its own block. Every
// ceiling test above spells the bound as `MAX_REVEAL` / `MAX_NEARBY_REVEAL`, so all of
// them pass just as happily at a million. Nothing else in the suite would notice the
// DOM bound being given back, or the reserve between the two segments being closed up.
describe('MAX_REVEAL and MAX_NEARBY_REVEAL', () => {
  it('is the whole-list bound the two segments divide, inside its DOM budget', () => {
    // This is about 22 nodes per card, measured in the running widget (331 rows
    // rendered 7,331 nodes), so 400 rows is about 8,800. This is an average over real
    // cards — chips and a distance line come and go — so treat it as a sizing
    // estimate, not a per-row invariant.
    //
    // The budget is ours, picked to sit just above the profiled-smooth depth. It is
    // not an external standard. Lighthouse's DOM audit warns an order of magnitude
    // lower, which this product cannot meet. Raising the ceiling means re-running
    // that profile and moving this number deliberately, which is the whole point of
    // failing here.
    const NODES_PER_ROW = 22
    const NODE_BUDGET = 10_000

    // This tests `MAX_REVEAL` alone. The reserve divides this budget. It does not add to it.
  ...
  it('leave the nearby segment STRICTLY below the ceiling', () => {
    ...
    // silently reinstates #129. See the reason in the `MAX_NEARBY_REVEAL` docblock.
    expect(MAX_NEARBY_REVEAL).toBeLessThan(MAX_REVEAL)
  })

  it('reserve enough for the distant segment to be worth reaching', () => {
    // This subsumes the strict check above at today's numbers, and both stay on
    // purpose. This one is a product floor — a one-row reserve is correct, but it
    // makes "Show distant events" reveal a single card. The check above is the
    // correctness ratchet, and it has to survive someone deciding the floor is
    // negotiable.
    expect(MAX_REVEAL - MAX_NEARBY_REVEAL).toBeGreaterThanOrEqual(PAGE_SIZE * 2)
  })

  it('stay far past any plausible reading depth', () => {
    // This checks the other direction. A ceiling of a page or two would be a
    // truncated product wearing a bound's clothes. This asserts on the nearby cap,
    // because that is now the binding one for the ordinary search. `MAX_REVEAL`
    // clears it by the reserve.
