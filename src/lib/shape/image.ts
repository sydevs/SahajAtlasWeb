...
//
// This returns null, instead of throwing or passing a raw value through,
// when the url cannot be resolved to an http(s) URL. That covers an empty
// or misconfigured origin (which makes `new URL` throw on a relative url),
// or a non-http(s) scheme (`data:`/`javascript:`). That keeps one bad image
// (or a bad env) from failing the whole event read, and stops a hostile CMS
// image url from smuggling a `</script>` payload into the OG/JSON-LD
// metadata. `getEvent` and the UI already skip null urls.
