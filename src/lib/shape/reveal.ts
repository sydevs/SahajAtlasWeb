...
// explicit press ("Show distant events"), which signposts that nearby events have run
// out. That press is the ONLY distance affordance — there is deliberately no "< N km"
// pill, so the automatic cut is never mistaken for one of the user's own filters.
 ...
 * than a URL param, so no link can jump straight to a huge one — but the ceiling still
 * matters: it is the only thing standing between a long enough session and a commit
 * that builds the whole matching feed, inside somebody else's page.
 *
 * This answers two separate questions, kept apart because issue #98 conflated them and
 * the measurements came out the other way round:
 *
 * **Why a ceiling at all — DOM hygiene in a page we do not own.** About 22,000 nodes at
 * the old 1,000 rows, about 8,900 at 400. That budget is ours, not an external
 * standard. Lighthouse's DOM audit starts warning an order of magnitude lower, which
 * the product genuinely cannot take.
 *
 ...
 * what makes a long list stutter. Profiled in the real vaul drawer at 6x CPU throttle
 * over an identical 40px-per-frame scroll, it is not:
 *
 ...
 * virtualizer: `content-visibility: auto` also defers per-row work to scroll-in, and
 * made the same list worse (p95 36→59ms, worst frame 61→307ms — a noisier run whose
 * own plain baseline was 36ms, so compare inside that pair rather than against the
 * 17.4ms above). Different mechanism, same direction. So windowing here is a *likely*
 * regression, not a measured one, and the ticket's sanctioned fallback was taken
 * instead.
 *
 * **A matching set larger than the ceiling ends here**, and the control disappears
 * rather than dead-ending, so it never lies about there being more. What a ceiling must
 * not do is end one SEGMENT by ending the list — applied to the combined count it did
 * exactly that, and `MAX_NEARBY_REVEAL` below is the reserve that stops it. This number
 * stays the bound on `rows.length` whichever segments are showing. That is the DOM
 * promise, and the ratchet in `reveal.test.ts` covers both constants, so neither can be
 * given back quietly.
 *
 ...
 * rather than trimming a list (issue #129). With enough nearby matches to reach
 ...
 * boundary became unreachable for that search — acknowledged only by the `sr-only`
 * live region, still counting to a total nothing could show. The nearby segment
 * AUTO-pages on scroll, so arriving there took no press at all.
 *
 * Reserving headroom fixes that **by construction, rather than by a special case**:
 * nearby paging stops with budget still unspent, so the crossing press always has room
 * to reveal rows into. That is the same no-op-free property every other `next` in this
 * file has, and it is why the reserve must be STRICT — `MAX_NEARBY_REVEAL < MAX_REVEAL`,
 * pinned in `reveal.test.ts`, since an equal pair silently reinstates the bug.
 *
 * An undivided list — no searched place, or every match nearby — has no second segment
 * to reserve for, so it keeps the whole budget and behaves exactly as it did before.
 *
 ...
 * rather than 400. Some tail is unreachable at any finite ceiling — this makes it the
 * far end of one segment, instead of the whole of the other.
 */
 ...
 * back to its first page. This is derived, rather than reset imperatively, so no call
 * site that changes one of these inputs can forget to clear the reveal.
 *
 * Built FROM the events query key, rather than re-deriving its parts, so the two
 ...
 * deliberately absent from the query key, but it does change which events the revealed
 * rows are.
 */
export const revealKey = (queryKey: readonly unknown[], sort: string): string =>
  // Uses `JSON.stringify`, not `join` — the key carries `filtersKey`, which embeds raw
  // URL values (a region slug, language tokens). Any separator those can contain would
  // let two different result sets collide on one key, and a collision hands one
 ...
 * the search point, and the country it sits in. This is structural, rather than
...
export type Reveal<T> = {
   ...
   * cannot reach for a reveal the list is not offering. Always strictly more rows than
   ...
   * to it ("showing 60 of 100"), and it has to include the distant segment, or the
 ...
 * not of an arbitrary nearest-N), and this function splits the result into the nearby
 * segment and the far one, preserving that order within each. Paging is clamped at the
 * segment boundary, so a press can never roll silently past "nearby" into events a
 * thousand km away.
 *
 ...
 * for events in another country. When it is absent — an ocean, a country-less feature,
 * a search that predates the param — every event is judged on distance alone.
 */
export function revealRows<T extends Segmentable>(
  ...
  // must be known to demote anything: an event with no address country (every online
  // one) is judged on distance alone, rather than on a fact we do not actually have.
  const limitFor = (event: T) => {
    // Uses `isoCountryCode`, like every other country consumer: the CMS address field
    // is a bare nullish string, so a malformed value (a 3-letter code, stray
    // whitespace) must degrade to "unknown" — and be judged on distance alone — rather
    // than compare as-is and demote an event on a fact we do not actually have.
  ...
  // The nearby segment gets its OWN ceiling whenever something sits past the boundary,
  // so paging through it always stops with budget left for the crossing — see
  // `MAX_NEARBY_REVEAL`. With no distant segment, there is nothing to reserve for, and
  // the list keeps the full ceiling.
  ...
  // them — spelled as the constant so this line cannot go stale when it moves), and
  // ceiling-gating on the raw count would then hide the control while a whole distant
  // segment sat unreached. It also keeps every `next` strictly greater than what is
  ...
  // holds back. This is a different number from `total`, because the two count
  // different things — the announcement counts MATCHES, `next` counts ROWS — so this
  // is what keeps a stored count describing rows that exist, rather than pointing past
  // the end of the list. It is NOT what keeps a press non-empty: `total` here would
  // never produce a no-op either (`more` is null whenever `revealed` has reached it).
  // The strict reserve is what makes the crossing press non-empty, and it is the only
  // thing that does.
  ...
  // below is distant. `null` only when nothing is left that a press could reveal —
  // offering one the clamp would undo is the one thing worse than stopping.
