 ...
 *
 * ⚠ **Since #217 the threat is the RESTRAINTS, not the credential.** A widget-graph write can
 * no longer send anything: the decorator that spends the token is in `request.ts`, outside that
 * graph, and the slot stays empty there. What such a write still does is open a session on a
 * page we do not own — every `<a>` inert, navigation snapping back, queries pinned to
