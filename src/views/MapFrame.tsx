 ...
 *
 * Map mode renders the canvas with `position: fixed; inset: 0`, and every drawer, peek strip,
 * and cog over it is fixed too. So by default it fills the browser window, whatever slot the
 * host gave `<sahaj-atlas>`, and #107 settled that as a documented requirement of the mode.
 * That requirement blocked the shape the WordPress plugin needs: a site's own header, then the
 * atlas below it. A static header got painted over. A sticky one floated above the map *and*
 * above the widget's drawers, covering the settings control.
 *
 ...
 * subtracts the frame's own top, which is zero when there is no frame.
 *
 * ⚠ **`contain: layout` belongs here, and NOWHERE up the tree.** `src/components/AGENTS.md`
 ...
 * with the host's page. Here re-parenting is the entire point, and this element belongs to the
 * widget alone.
 *
 * **It also gives the host a defined stacking relationship.** Layout containment makes this a
 * stacking context, so the widget's internal `z-30`…`z-50` stop competing with whatever the
 * host's CSS happens to produce, and the host's page orders the atlas as one element — which is
 * why a sticky header can float over it correctly, instead of over its drawers.
 *
 * **Rendering nothing when `contained` is false is deliberate**, rather than an inert wrapper.
 * An always-on frame would take the containing block on every map embed, including the
...
function ContainedFrame({ children }: { children: ReactNode }) {
   ...
   * for `min-height: 640px`. Meanwhile `height: 100%` against that same element resolves to
   * `auto`, because percentage heights resolve against the parent's computed `height`, not its
   * used one. The frame then computes to 0px, and since it carries `contain: layout`, the whole
   * fixed layer resolves against a zero-height containing block. The map, the drawers, and the
   * cog all vanish, while the readiness marker still attests a healthy embed.
   *
   * That is also a REGRESSION if left unhandled, which is what makes reporting insufficient on
   * its own. Before #169, the very same page rendered the compact card — a working button and
   * the right advice. So an unfillable box gets refused outright, and the embed falls back to
   * the viewport map it would have had, with one line saying which rule to change.
   *
   * The check runs in the callback ref, during the layout phase. The div is in the DOM with its
   * final box for that pass. `offsetHeight` forces the flush that makes the read correct. The
   * children stay gated on `node`, so nothing has rendered inside the broken frame yet.
   */
  ...
  return (
    // `h-full` resolves against `<sahaj-atlas>`. The theme root between them is `display:
    // contents` and generates no box, so the host's own height is what the widget fills,
    // exactly as in `map=false`. `overflow-hidden` keeps a promise. Nothing the interface
