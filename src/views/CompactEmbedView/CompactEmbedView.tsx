...
/**
 * What the widget IS in a slot too small for the interface (issue #161): a card with
...
function Card({ action }: { action: CardAction }) {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-2 overflow-hidden bg-background p-3 text-foreground">
      {/* Capped, rather than full-bleed: a button stretched across a 1000px-wide slot
      ...
      <div className="flex w-full max-w-xs flex-col gap-2">
        ...
        {/* Two JSX branches, not a conditional `href`: `ButtonProps` is a discriminated
            ...
            <a> — `href.test.ts` pins the app's JSX-anchor inventory to three components,
            and Button is one of them, so its `isSafeHref` gate is inherited rather than
            reimplemented. */}
        {action.kind === 'link' ? (
 ...
 *
 * **It keeps a margin, and the margin is the point.** A full-bleed `inset: 0` layer
 * reads as a navigation — the host's page simply gone — and leaves a visitor no way
 * to judge they are still on it. A visible frame of the page behind, plus
 * click-outside-to-close, makes it legibly a layer over their site.
 *
...
function ExpandedDialog({
  ...
  //
  // **Radix's own restore targets `Primitive.Trigger`, and there is deliberately
  // none here**: opening is requested through the `useExpansion` seam, from a
  // control that may be anywhere. With no trigger, Radix focuses nothing, so a
  // keyboard viewer who closes is dropped on `<body>` — the top of the HOST's page,
  // not the control they pressed.
  //
    ...
    <Primitive.Root open={open} onOpenChange={onOpenChange}>
      ...
      <Primitive.Portal container={widgetOverlayContainer()}>
        ...
        <Primitive.Content
          ...
          // Focuses the container, not the first control inside it. Radix's default
          // sends focus to the first tabbable — here, whatever chrome the interface
          // renders first, announced with no word about what just opened.
        ...
        >
          ...
          >
            ...
            {node && children}
            {/* The pointer's exit. No longer the only one — Escape reaches this component through
...
export type CompactEmbedViewProps = {
   ...
   *
   * Passed as `children`, rather than constructed here, so React never renders it
   * while collapsed — which is what keeps mapbox-gl unfetched behind a compact
   * embed, and makes that claim true rather than aspirational.
   */
  children: ReactNode
}

export function CompactEmbedView({ compact, children }: CompactEmbedViewProps) {
  // A `link` card has nothing to open: the button is an anchor to a page that fits,
  // because a framed embed cannot grow where it is. No provider state, no dialog —
  // and `NoExpansionProvider` is what keeps `collapse()` an honest no-op for the
  // Escape ladder in `DrawerStack`.
    ...
    <>
      ...
      <ExpandedDialog
        ...
        onOpenChange={(next) => {
          ...
          // `ReportIssueModal` is mounted OUTSIDE this branch (it has to outlive
          // the app boundary so the error fallbacks can reach it) but portals
          // through `overlayContainer()`, which is this dialog while it is open.
          // Closing with the modal up would leave it rendering into a detached
          // node with its own scroll lock still on the host page.
