 ...
 * no USABLE report came back — reported as "unavailable", which is a different
 * failure from "found something".
 *
 ...
 * `advisories` and no `metadata`, and so reads downstream as a clean tree with
 * nothing to report — a green gate on an audit that never ran. It was masked
 * while the baseline was non-empty (zero findings against a 27-entry list
 * tripped the guard below); emptying the baseline in #101 exposed it. So both
 * the error envelope and a missing `metadata` are rejected here, at the one
 * place that already knows how to say "unavailable".
 */
function runAudit() {
  let stdout = ''
 ...
 * settings from `pnpm-workspace.yaml`, so a major bump needs this widened or
 * the check goes quietly blind.
 */
...
/**
 * Append the table of advisories that are NOT in the baseline, if any.
 *
 * Shared by the normal path and the blind-run path: a run that could not be
 * trusted as clean still has to print what it did manage to see, or the warning
 * replaces the finding instead of accompanying it.
 */
...
function main() {
  ...
  //    SOURCE, before this file sees them. Now that the baseline is empty it is
  //    the only remaining way to hide a finding, and unlike a baseline line it
  //    records no owner and no ticket. Its presence is therefore the failure, not
  //    a thing to go and inspect — which is why it reds a PR rather than warning
  //    like the other two. It is a repo-authored change, visible in the diff of
  //    whoever added it; a registry outage is not anybody's PR.
  ...
  //    A registry outage is NOT this case — its error envelope is rejected by
  //    `runAudit`, which routes it to the "unavailable" branch above.
  ...
  //
  // Whatever the reason, a blind run must not also SWALLOW what it did see: the
  // new-advisory table is printed and its exit code respected first, or a single
  // suppression entry would turn every unrelated finding green at once.
