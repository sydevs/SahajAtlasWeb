 ...
 *
 * Per-chunk sizes mean nothing here: this build re-chunks freely (rolldown), so
 * moving code between chunks can shrink any single file while the payload a
 * visitor actually downloads stays unchanged. The only number worth budgeting is
 ...
 *      emits **zero** modulepreload hints (nothing writes an HTML shell for
 *      it), so a check that only read `index.html` would never see the
 *      embed's cost at all — and the embed is the actual product.
 *
 * This excludes dynamic `import()` targets — but read that carefully, because it
 * hid a real regression for the length of one PR. A dynamic import is only a
 * second hop when the module HOLDING it is itself lazy. `react-map-gl` runs
 * `const mapLib = import('mapbox-gl')` at module scope, so while anything in the
 * eager graph imported it — including one re-export through the `organisms`
 * barrel — 485 KiB gz was fetched concurrently with the payload, and this gate
 * reported a comfortable number. When adding a heavy dependency, check what its
 * module BODY does, not just how the app imports it. The eager graph alone keeps
 * this check honest about what it claims to measure.
 *
 * ## About the numbers
 *
 * This uses gzip at zlib's default level, matching the readiness-report
 * measurement and Vite's own build report. A CDN may use a different level or
 * brotli entirely, so treat these numbers as a *regression detector*
 * (comparable build-to-build), not as bytes-on-the-wire.
 */
...
// unnoticed *growth* without failing the day it lands. Headroom is about 3%:
// enough to absorb a dependency patch bump, tight enough that a newly-eager
// view trips it — and comfortably above the roughly 2.1 KiB a credentialed
// build adds that CI cannot see (see SLACK_FLOOR_KIB below).
//
//   standalone  297.2 KiB  →  308
//   embed       299.8 KiB  →  308
//
// Ratcheted here by the Lucide icon swap (#003), which took about 3 KiB off
// both graphs: the tree-shaken glyphs cost less than the hand-drawn path data
// they replaced, even after adding a dependency. Note the direction — a swap
// that ADDS a package can still win space, and the ratchet is what stops that
// win being quietly re-spent.
//
// The previous line was 310/310 against 299.3/301.3 (issue #161, which took
// about 83 KiB off both graphs by moving the whole map-bearing interface
// behind a lazy `FullInterface` seam — that is what makes a compact embed
// cheap: a slot too small for the interface mounts a card and one dialog, and
// never fetches mapbox-gl at all). Before that, 395/395 against 382.1/383.6
// (issue #96, which took 99.7 KiB off by lazying the calendar, registration,
// and share drawers). Before that 495/495 against 479.5/480.4 (issue #99).
//
// RATCHET THIS DOWN again whenever the payload shrinks, or the budget quietly
// re-accumulates the slack it just won. `SLACK_RATIO` below enforces that,
...
// far UNDER budget fails too, and the fix is one line: bring BUDGET_KIB down
// in the commit that won the space.
//
...
// A FLOOR under the slack allowance, in KiB, without which the ratchet rule and
// the leave-headroom-for-Sentry rule are mathematically unsatisfiable on a
// small graph.
//
// `AGENTS.md` records that a credentialed build ships more than CI measures,
// because `@sentry/vite-plugin` injects a debug-ID snippet per chunk and CI has
// no token. Measured on this build: +2.1 KiB on `standalone` and `embed`, and
// **+0.5 KiB on `loader`**. The absolute cost is small. The RATIO is not,
// because it scales with chunk count rather than bytes — 0.5 KiB is 17% of the
// 3.0 KiB loader graph, where it is 0.6% of the embed's.
//
// So for a graph this small the two rules collide head-on. A budget high
// enough to clear the credentialed build (above 3.5) leaves spare that a flat
// 15% then calls "far under budget", and the only numbers satisfying both sit
// in a window about 0.03 KiB wide. That is not a budget, it is a knife edge,
// and the next person to touch the loader would have hit it.
//
// The floor resolves it without weakening anything: on the big graphs, 15% is
// far larger and still governs, so their ratchet behaviour stays unchanged. It
// only applies where the percentage would be tighter than the known,
// unavoidable delta between what CI can measure and what production ships.
 ...
 * and the bare side-effect `import "./x.js"`. A dynamic `import("./x.js")` has
 * a paren before the quote and so matches neither — which is the point.
 * Anything that resolves to a file not in `dist/` is discarded, so a stray
 * string literal that happens to look like a specifier cannot inflate the
 * total.
 */
 ...
 *
 * Existence alone is not the claim the docblock above makes: `../../../etc/hosts`
 * both resolves and exists, and would then be gzipped and named in the summary.
 * Containment is what makes "a stray literal cannot inflate the total" true.
 */
...
function measure(name, files) {
  ...
  // Without this check, an unbudgeted graph makes `spare` NaN, and NaN fails
  // every comparison below — so a new entry would report `undefined KiB` and
  // pass.
  if (budget === undefined) {
...
function main() {
  ...
  // Names the paths: if the build is fine and one of these was renamed (the
  // embed filename is `entryFileNames` in vite.config.ts), the message points
  // at the rename, rather than sending someone to rebuild a build they just
  // ran.
  ...
  // The shell's own list is what the readiness report measured, but the
  // closure walked from the entry is what the browser actually pulls. They
  // agree today. Scoring their UNION means a dropped preload hint can never
  // make the payload look smaller than it is — it can only show up as the
  // warning below.
  ...
  // This is the seam the whole loader split rests on: `embed.js` must be reachable from `auto.js`
  // only through a DYNAMIC import, never a static one. Rolldown would happily hoist it into the
  ...
  // This fails, rather than nags. A notice here would leave the budget
  // describing a payload that no longer exists, which is the same silent
  // green the whole script is aimed at — and it is also what a half-broken
  // import walker looks like, where a pass would be actively false.
  if (under.length) {
    annotate(
      'error',
