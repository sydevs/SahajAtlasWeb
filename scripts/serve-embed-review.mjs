...
/**
 * Serves the built widget as a REAL embed, on host pages, for browser
 * review.
 *
 *   pnpm build && pnpm review:embed
 *
 * Browser verification is where this repo's embedding bugs actually get
 * found. Issue #161's inverted bottom sheet, its collapsed card fetching
 * the whole feed, and the `all: revert` rule that erased every icon were
 * all invisible to lint, typecheck, and a green unit lane. This script
 * exists because standing up that environment by hand costs a dozen
 * turns, and it has four traps in it. The session that produced this
 * script hit every one of them.
 *
 * ## The four traps, and how this script avoids them
 *
 * **1. `<sahaj-atlas>` observes NO attributes.** Config rides on the
 * *script URL* — `auto.js?key=…&map=…` — and this widget parses it once
 * at load (`src/loader/config.ts`). A hand-written
 * `<sahaj-atlas apikey="…" map="true">` is inert. The element mounts, the
 ...
 * request for a host page that server does not have returns **200 with
 * the app shell**, and a reviewer reads the wrong document without
 * noticing. This script refuses to start on an occupied port, rather
 * than assuming the occupant is ours.
 *
 ...
 * 2026-08-20, the seeded local backend answers `403` — "Not
 * authenticated as an Atlas client" — for both keys in `.env.local`. So
 * the interface falls through to its error panel. The COMPACT CARD is
 * unaffected in principle, since it renders before any of that, but
 * confirm rather than assume. What needs no backend at all: which chunks
 * are fetched (mapbox-gl in particular), the slot decision's console
 * message, CSS scoping, and icon rendering.
 */
...
/**
 * This uses Vite's own env loader, not a hand-rolled one. It applies
 * `.env` then `.env.local` precedence, and it handles quoting and inline
 * comments correctly. A 15-line reader here once got both subtly wrong.
 * `fallback-url.ts` on this same branch is the cautionary tale for
...
/**
 * ⚠ `full` is not cosmetic. The normal shell centers content in a
 * `max-width: 1100px` column, which makes `<sahaj-atlas>`'s parent
 * 1164px wide. An empty custom element measures 0×0, so that parent IS
 * the slot `decideSlot` reads. Above a roughly 1455px viewport, 1164 is
 * meaningfully smaller than the window. So a MAP page rendered in the
 * normal shell correctly resolves to the compact card, and shows no map
 * at all. The one page whose whole purpose is "map mode owns the
 * viewport" has to be full-bleed, or it demonstrates the opposite of its
 * own note on any large monitor.
 */
...
function pages(src) {
  ...
  return {
    // The default product. Map mode owns the viewport. At phone width,
    // the drawer is a bottom sheet with snap points — the shape whose
    // offsets #161 inverted, by handing vaul a `display: contents`
    // element to measure. Resize to about 390px wide to see it.
    'full-map.html': page(
      'Full-page map embed',
    ...
    // This is the shape issue #169 exists for: a site's own sticky
    // header, then the atlas below it. The element carries a height,
    // which is the whole opt-in. `MapFrame` takes the containing block
    // with `contain: layout`, so the map, its drawers, and its peek
    // strips all resolve against this box instead of the window. Check
    // two things: the header must stay visible and on top while
    // scrolling, and NOTHING the widget renders may paint outside the
    // bordered box.
    ...
    // A slot too small for the interface: the compact card. Note the
    // height. Since #169, a height makes this a CONTAINED map. So the
    // card here is earned by the 300px width sitting under the interface
    // floor, and the console says so. A map embed with no height of its
    // own is the one that still gets the viewport-ownership sentence.
    // Watch that no mapbox chunk is fetched, and that only `clients/me`
    // is requested while the card is closed.
    ...
    // fills the frame. Below the interface floors, the card appears, and
    // its button opens the fallback in a NEW TAB.
    'iframe.html': page(
      'Iframe embeds',
    ...
    // Path routing. ⚠ This page is the ONE exception to trap 4's 404
    // rule. The server serves it for everything under
    // `/__review/pathmode`, because that wildcard IS what path routing
    // asks of a host. The exception is narrow on purpose: exactly this
    // prefix, nothing else.
    //
    // This still falls back to query routing until a client record
    // carries a matching `canonical.embed`
    // (`localhost:<port>/__review/pathmode`), which the seeded backend
    // does not. To see path mode actually engage, stub `clients/me` —
    // see `docs/rules/mapbox.md`.
    'pathmode.html': page(
      'Path routing',
    ...
    // A deep link on the PAGE url: eager mount, plus auto-open. The
    // script-URL `atlas` parameter is a configured default, and it must
    // do neither.
    'deep-link.html': page(
...
// This index page is served from memory, never written to disk. An
// earlier version wrote the index into `dist/`, which left a stray file
// for `assert-no-sourcemaps` to scan on the next build. It also implied
// the same-origin property came from living there. It actually comes
// from the server, as the other six pages already demonstrate.
const index = page(
  'Embed review',
  '<ul>' +
    Object.keys(written)
...
const server = createServer((req, res) => {
  ...
  try {
    ...
    // `decodeURIComponent('/%')` throws. An unguarded throw in this
    // listener becomes an uncaughtException that kills the server,
    // leaving a dead port and no message. That is trap 4 wearing a
    // different hat.
  ...
  // ⚠ This is the single wildcard, and it stays single. Path routing
  // requires the host to serve one document for a whole subtree, so the
  // page demonstrating it cannot be reached any other way. But a general
  // fallback here would re-arm trap 4, which is the whole reason this
  // server returns 404 for everything else.
  ...
  // ⚠ This checks `isFile`, not only `existsSync`. A directory EXISTS,
  // so `existsSync` alone waves it through. This code would then send a
  // 200, and `createReadStream(dir)` would emit an unhandled `EISDIR` —
  // an uncaughtException that kills the server. `/assets` and `/locales`
  // are both real directories in `dist/`, so one speculative fetch was
  // enough to hit this. It is the same dead-port-and-no-message failure
  // as the malformed escape above, and it was the second of the two
  // crash paths.
...
// ⚠ Loopback ONLY. Without the host argument, Node binds `0.0.0.0`, and
// every page here has the client API key substituted into it. So on
// shared Wi-Fi, the whole subnet could read the key and browse `dist/`.
// The key is a published client credential rather than a secret, but
// there is no reason to hand it out. A bind that answers only
